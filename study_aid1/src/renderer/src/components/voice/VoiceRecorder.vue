...
const handleStartRecording = async () => {
  ...
  try {
    ...
    // Use a queue to avoid blocking the audio thread
    ...
    const processQueue = async () => {
      ...
      while (audioQueue.length > 0) {
        ...
        if (data) {
          try {
            await window.electronAPI.speechRecognition?.sendAudio?.(data)
          } catch (error) {
    ...
    processor.onaudioprocess = (e) => {
      ...
      audioQueue.push(uint8Data)

      // 只在开始时打印日志，避免日志过多
      if (frameCount <= 5 || frameCount % 50 === 0) {
        console.log('[VoiceRecorder] 音频帧入队:', {
          frame: frameCount,
    ...
    // NOTE: gainNode must be created BEFORE storing in mediaRecorder object
    const gainNode = audioContext.createGain()
    gainNode.gain.value = 0  // Mute the output to prevent feedback

    // Store for cleanup (now gainNode is initialized)
    mediaRecorder = {
      stream,
      audioContext,
      source,
...
const handleStopRecording = async () => {
  // IMPORTANT: Set isRecording = false FIRST to stop onaudioprocess callback
  isRecording.value = false

  // Stop counters and visualizer
  stopDurationCounter()
  stopAudioVisualizer()
