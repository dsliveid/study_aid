...
```bash
# 通过命令行调用
./main.exe -m models/ggml-base.en.bin -f audio.wav
```
...
#### 方案二：Python + whisper.cpp（高性能与高准确率方案）

**技术栈**
- 开发语言：Python 3.8+
- 音频采集：pyaudio / sounddevice
- 系统音频：pyaudio + WASAPI
- 语音识别：whisper.cpp (通过命令行调用)
- UI框架：PyQt5
- 核心库：subprocess, numpy
...
class SpeechRecognizer:
    ...
    def recognize_audio_chunk(self, audio_chunk: np.ndarray, sample_rate: int):
        """识别单个音频块"""
        temp_audio_path = "temp_audio.wav"
        sf.write(temp_audio_path, audio_chunk, sample_rate)
        
        def recognition_task():
            transcription = self._run_whisper_cpp(temp_audio_path)
            if transcription:
                self.result_queue.put(transcription)

        self.recognize_thread = threading.Thread(target=recognition_task)
