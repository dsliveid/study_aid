...
### 4.1 whisper.cpp 功能测试

#### TC-SR-001: 基本转录功能测试

**测试目的**：验证 `whisper.cpp` 的基本转录功能。

**前置条件**：

- `whisper.cpp` 的可执行文件 `main.exe` 已存在。
- `ggml-base.bin` 模型文件已下载并放置在 `whisper_cpp` 目录下。
- 用于测试的音频文件 `recorded_audio_16khz.wav` 已存在。

**测试步骤**：

1. 构造调用 `main.exe` 的命令行，包含模型和音频文件路径。
2. 执行命令行。
3. 检查命令是否成功执行，并读取生成的 `.txt` 输出文件。
4. 验证转录结果是否包含预期的文本。
5. 清理生成的文本文件。
...
class SpeechRecognitionTest(unittest.TestCase):
    ...
    def test_basic_transcription_whisper_cpp(self):
        ...
        # 检查所需文件是否存在
        self.assertTrue(os.path.exists(exe_path), f"可执行文件不存在: {exe_path}")
        ...
        # 构造命令行
        # main.exe -m <model_path> -f <audio_path> -otxt
        ...
        # 读取生成的文本文件
        output_txt_path = os.path.splitext(audio_path)[0] + ".wav.txt" # main.exe 会附加 .wav.txt
        ...
        self.assertIn(normalized_expected, normalized_result, f"转录结果与预期不符: 得到 '{result_text}', 预期包含 '{expected_text}'")
...
| 模块     | 测试用例数 | 通过       | 失败       | 通过率      |
| -------- | ---------- | ---------- | ---------- | ----------- |
| 音频采集 | 7          | \_\_\_     | \_\_\_     | \_\_\_%     |
| 音频处理 | 5          | \_\_\_     | \_\_\_     | \_\_\_%     |
| 语音识别 | 5          | \_\_\_     | \_\_\_     | \_\_\_%     |
| 集成测试 | 1          | \_\_\_     | \_\_\_     | \_\_\_%     |
| 性能测试 | 2          | \_\_\_     | \_\_\_     | \_\_\_%     |
| **总计** | **20**     | **\_\_\_** | **\_\_\_** | **\_\_\_%** |

### 性能指标

| 指标       | 目标值   | 实际值 | 状态   |
| ---------- | -------- | ------ | ------ |
| 识别延迟   | < 10s    | \_\_\_ | \_\_\_ |
| 峰值内存   | < 2000MB | \_\_\_ | \_\_\_ |
| 峰值CPU    | < 100%   | \_\_\_ | \_\_\_ |
| 识别准确率 | > 90%    | \_\_\_ | \_\_\_ |
